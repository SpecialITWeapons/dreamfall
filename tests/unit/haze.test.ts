...
describe('hazeAt', () => {
  ...
  it('weighs a tint by how much of the biome is under the flyer', () => {
    expect(hazeAt([0, 1, 2], [1, 0, 0], specs, 0, out)).toBeCloseTo(0.3, 6);
    expect(out.getHex()).toBe(green.color.getHex());
    expect(hazeAt([0, 1, 2], [0.5, 0, 0], specs, 0, out)).toBeCloseTo(0.15, 6);
    // and a biome that asked for nothing tints nothing
    expect(hazeAt([2, 2, 2], [1, 0, 0], specs, 0, out)).toBe(0);
  });

  it('averages two tints instead of adding them up', () => {
    // Half and half: the air is the colour between, not twice as thick.
    const amount = hazeAt([0, 1, 2], [0.5, 0.5, 0], specs, 0, out);
  ...
  it('is low air: gone from over it, and never more than a tint', () => {
    expect(hazeAt([0, 1, 2], [1, 0, 0], specs, 300, out)).toBeCloseTo(0.3, 6);
    expect(hazeAt([0, 1, 2], [1, 0, 0], specs, 900, out)).toBeLessThan(0.2);
    expect(hazeAt([0, 1, 2], [1, 0, 0], specs, 1600, out)).toBe(0);
    ...
    expect(hazeAt([0, 0, 0], [1, 1, 1], greedy, 0, out)).toBeCloseTo(MAX_HAZE, 6);
  });
});
